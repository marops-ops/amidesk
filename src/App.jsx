...
function OthersCampaignPage({tasks, customers, teamMembers, session, navigate, updateCampaign, adjustBank, logActivity}) {
  ...
  const assignAllCampaigns=async(custTasksToAssign,staff)=>{
    const {data:profile}=await sb.from("profiles").select("id").eq("email",staff.email).single();
    if(!profile){alert(staff.name+" har ikke logget inn i AmiDesk ennå.");return;}
    for(const t of custTasksToAssign){
      await sb.from("campaigns").update({owner_id:profile.id}).eq("id",t.id);
      logActivity&&logActivity(t.customerId,t.id,"campaign_transferred",`Kampanje "${t.title}" overført til ${staff.name} (del flere)`);
    }
    const senderName=session?.user?.user_metadata?.full_name||session?.user?.email||"Noen";
    await sb.from("notifications").insert({id:uid(),user_id:profile.id,type:"campaign_given",message:senderName+" ga deg kampanjer for en kunde",brief_id:null,read:false});
    setShareModalTarget(null);
    alert("Overført! Oppdatering vises ved neste innlasting av siden.");
  };
  ...
  return (
    <div>
      ...
      {shareModalTarget&&<ShareCampaignsModal custTasks={shareModalTarget.tasks} custName={shareModalTarget.name}
        onClose={()=>setShareModalTarget(null)} onShare={shareAllCampaigns} onAssign={assignAllCampaigns}/>}
    </div>
  );
}
...
function AddLineModal({task, channel, onClose, onSave}) {
  ...
  const [useAdGroups,setUseAdGroups]=useState(false);
  const [adGroups,setAdGroups]=useState([{id:uid(),name:"",budget:"",start:task.start||"",end:task.end||""}]);

  const addAdGroup=()=>setAdGroups(p=>[...p,{id:uid(),name:"",budget:"",start:task.start||"",end:task.end||""}]);
  const removeAdGroup=id=>setAdGroups(p=>p.filter(g=>g.id!==id));
  const updateAdGroup=(id,field,val)=>setAdGroups(p=>p.map(g=>g.id===id?{...g,[field]:val}:g));

  const adLabel=adGroupLabel(channel);
  const total=useAdGroups
    ?adGroups.reduce((a,g)=>a+(+g.budget||0),0)
    :(+budget||0);

  const save=()=>{
    if(!name) return alert("Fyll inn linjenavn");
    if(total<=0) return alert("Legg inn budsjett");
    const ch=channel;
    if(useAdGroups) {
      const lines=adGroups
        .filter(g=>+g.budget>0)
        .map(g=>({
          key:ch+" — "+name+" / "+(g.name||"Ad group"),
          budget:+g.budget,
          start:g.start,
          end:g.end,
        }));
      if(!lines.length) return alert("Legg inn budsjett på minst ett "+adLabel.slice(0,-1));
      onSave(lines);
    } else {
      onSave([{key:ch+" — "+name, budget:+budget, start, end}]);
    }
  };
    ...
    <div className="modal-overlay" onClick={e=>e.target===e.currentTarget&&onClose()}>
      <div className="modal" style={{maxWidth:560,maxHeight:"92vh"}}>
        ...
        <div style={{marginBottom:12}}>
          <label>Linjenavn / kampanjenavn</label>
          <input value={name} onChange={e=>setName(e.target.value)} placeholder="f.eks. Trafikk" autoFocus/>
        </div>

        {!useAdGroups&&(
          <>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:12}}>
              <div><label>Startdato</label><input type="date" value={start} onChange={e=>setStart(e.target.value)}/></div>
              <div><label>Sluttdato</label><input type="date" value={end} onChange={e=>setEnd(e.target.value)}/></div>
            </div>
        ...
        <label style={{display:"flex",alignItems:"center",gap:7,cursor:"pointer",marginBottom:useAdGroups?12:20}}>
          <input type="checkbox" checked={useAdGroups} onChange={e=>setUseAdGroups(e.target.checked)} style={{width:"auto"}}/>
          <span style={{fontFamily:"Roboto,sans-serif",fontSize:12,color:C.ink3}}>Legg til {adLabel.toLowerCase()}</span>
        </label>

        {useAdGroups&&(
          <div style={{marginBottom:16}}>
            <div style={{fontFamily:"Roboto,sans-serif",fontSize:10,letterSpacing:".07em",textTransform:"uppercase",color:C.ink4,marginBottom:8}}>{adLabel}</div>
            {adGroups.map((g,i)=>(
              <div key={g.id} style={{background:C.cardAlt,borderRadius:9,padding:"10px 12px",marginBottom:8,border:"1px solid "+C.borderSoft}}>
                <div style={{display:"grid",gridTemplateColumns:"1fr 100px auto",gap:8,marginBottom:8,alignItems:"center"}}>
                  <input value={g.name} onChange={e=>updateAdGroup(g.id,"name",e.target.value)} placeholder={adLabel.slice(0,-1)+" "+(i+1)} style={{fontSize:12}}/>
                  <input type="number" value={g.budget||""} onChange={e=>updateAdGroup(g.id,"budget",e.target.value)} placeholder="0" style={{textAlign:"right",fontSize:12}}/>
                  {adGroups.length>1&&<button className="btn" onClick={()=>removeAdGroup(g.id)} style={{background:"none",color:C.badFg,padding:"3px 6px",border:"1px solid "+C.badBg,borderRadius:7}}><X size={11}/></button>}
                </div>
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
                  <div><label style={{fontSize:9}}>Startdato</label><input type="date" value={g.start} onChange={e=>updateAdGroup(g.id,"start",e.target.value)} style={{fontSize:11}}/></div>
                  <div><label style={{fontSize:9}}>Sluttdato</label><input type="date" value={g.end} onChange={e=>updateAdGroup(g.id,"end",e.target.value)} style={{fontSize:11}}/></div>
                </div>
              </div>
            ))}
            <button className="action-btn" onClick={addAdGroup}><Plus size={11}/> Legg til {adLabel.slice(0,-1).toLowerCase()}</button>
            {total>0&&<div style={{fontFamily:"Roboto,sans-serif",fontSize:12,color:C.ink3,marginTop:8}}>Total: <strong style={{color:C.ink}}>{fmtNOK(total)}</strong></div>}
          </div>
        )}
...
function AddAdGroupModal({task, parentFlatKey, parentLabel, channel, onClose, onSave}) {
  const adLabel=adGroupLabel(channel);
  const singular=adLabel.slice(0,-1).toLowerCase();
  const [groups,setGroups]=useState([{id:uid(),name:"",budget:"",start:task.start||"",end:task.end||""}]);
  const addGroup=()=>setGroups(p=>[...p,{id:uid(),name:"",budget:"",start:task.start||"",end:task.end||""}]);
  const removeGroup=id=>setGroups(p=>p.filter(g=>g.id!==id));
  const updateGroup=(id,field,val)=>setGroups(p=>p.map(g=>g.id===id?{...g,[field]:val}:g));
  const total=groups.reduce((a,g)=>a+(+g.budget||0),0);

  const save=()=>{
    const lines=groups.filter(g=>+g.budget>0).map((g,i)=>({
      key:parentFlatKey+" / "+(g.name||adLabel.slice(0,-1)+" "+(i+1)),
      budget:+g.budget, start:g.start, end:g.end,
    }));
    if(!lines.length) return alert("Legg inn budsjett på minst én "+singular);
    onSave(lines);
  };

  return (
    <div className="modal-overlay" onClick={e=>e.target===e.currentTarget&&onClose()}>
      <div className="modal" style={{maxWidth:520,maxHeight:"92vh"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}>
          <h2 style={{fontFamily:"'Montserrat',sans-serif",fontSize:18,fontWeight:600,color:C.ink}}>Ny {singular} — {parentLabel}</h2>
          <button className="btn" onClick={onClose} style={{background:"none",color:C.ink3,padding:"4px"}}><X size={18}/></button>
        </div>
        <div style={{fontFamily:"Roboto,sans-serif",fontSize:11,color:C.ink3,marginBottom:14}}>
          Legges til på denne kampanjen. Budsjettet trekkes som nye kroner fra banken. Vil du heller flytte penger fra en pauset {singular}, reduser budsjettet der først (refunderes automatisk), og legg til her etterpå.
        </div>
        {groups.map((g,i)=>(
          <div key={g.id} style={{background:C.cardAlt,borderRadius:9,padding:"10px 12px",marginBottom:8,border:"1px solid "+C.borderSoft}}>
            <div style={{display:"grid",gridTemplateColumns:"1fr 100px auto",gap:8,marginBottom:8,alignItems:"center"}}>
              <input value={g.name} onChange={e=>updateGroup(g.id,"name",e.target.value)} placeholder={adLabel.slice(0,-1)+" "+(i+1)} style={{fontSize:12}} autoFocus={i===0}/>
              <input type="number" value={g.budget||""} onChange={e=>updateGroup(g.id,"budget",e.target.value)} placeholder="0" style={{textAlign:"right",fontSize:12}}/>
              {groups.length>1&&<button className="btn" onClick={()=>removeGroup(g.id)} style={{background:"none",color:C.badFg,padding:"3px 6px",border:"1px solid "+C.badBg,borderRadius:7}}><X size={11}/></button>}
            </div>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
              <div><label style={{fontSize:9}}>Startdato</label><input type="date" value={g.start} onChange={e=>updateGroup(g.id,"start",e.target.value)} style={{fontSize:11}}/></div>
              <div><label style={{fontSize:9}}>Sluttdato</label><input type="date" value={g.end} onChange={e=>updateGroup(g.id,"end",e.target.value)} style={{fontSize:11}}/></div>
            </div>
          </div>
        ))}
        <button className="action-btn" onClick={addGroup}><Plus size={11}/> Legg til {singular}</button>
        {total>0&&<div style={{fontFamily:"Roboto,sans-serif",fontSize:12,color:C.ink3,margin:"10px 0"}}>Nytt budsjett: <strong style={{color:C.ink}}>{fmtNOK(total)}</strong> (trekkes fra bank)</div>}
        <button className="btn" onClick={save} style={{background:C.sand,color:"#fff",padding:"11px",borderRadius:9,fontFamily:"Roboto,sans-serif",fontSize:13,width:"100%",marginTop:8}}>
          Legg til {singular}
        </button>
      </div>
    </div>
...
function CampaignPage({tasks, customers, updateCampaign, deleteCampaign, navigate, adjustBank, onAddCampaign, briefs=[], setShowCreateBrief, isAdmin, session, customerOrder=[], onReorder, logActivity, teamMembers=[]}) {
  ...
  const assignAllCampaigns=async(custTasksToAssign,staff)=>{
    ...
    if(!profile){alert(staff.name+" har ikke logget inn i AmiDesk ennå.");return;}
    for(const t of custTasksToAssign){
      await updateCampaign(t.id,{ownerId:profile.id});
      logActivity&&logActivity(t.customerId,t.id,"campaign_transferred",`Kampanje "${t.title}" overført til ${staff.name} (del flere)`);
    }
    const senderName=session?.user?.user_metadata?.full_name||session?.user?.email||"Noen";
    await sb.from("notifications").insert({id:uid(),user_id:profile.id,type:"campaign_given",message:senderName+" ga deg kampanjer for en kunde",brief_id:null,read:false});
  ...
  const [addAdGroupTarget, setAddAdGroupTarget] = useState(null);
  ...
  return (
    <div>
      ...
      {addLineTarget&&<AddLineModal
        ...
        onSave={(lines, channelDates)=>{
          const newBudgets={...addLineTarget.task.channelBudgets};
          const newDates={...(addLineTarget.task.channelDates||{})};
          let totalAdded=0;
          lines.forEach(({key,budget,start,end})=>{
            newBudgets[key]=budget;
            totalAdded+=budget;
            if(start||end) newDates[key]={start:start||addLineTarget.task.start,end:end||addLineTarget.task.end};
          });
          const newTotal=Object.entries(newBudgets).filter(([k])=>!k.endsWith("__parent__")).reduce((a,[,b])=>a+b,0);
          updateCampaign(addLineTarget.task.id,{channelBudgets:newBudgets,channelDates:newDates,budget:newTotal});
          adjustBank&&adjustBank(addLineTarget.task.customerId,-totalAdded);
          setAddLineTarget(null);
        }}
      />}
      {addAdGroupTarget&&<AddAdGroupModal
        task={addAdGroupTarget.task}
        parentFlatKey={addAdGroupTarget.parentFlatKey}
        parentLabel={addAdGroupTarget.parentLabel}
        channel={addAdGroupTarget.channel}
        onClose={()=>setAddAdGroupTarget(null)}
        onSave={(lines)=>{
          const newBudgets={...addAdGroupTarget.task.channelBudgets};
          const newDates={...(addAdGroupTarget.task.channelDates||{})};
          let totalAdded=0;
          lines.forEach(({key,budget,start,end})=>{
            newBudgets[key]=budget;
            totalAdded+=budget;
            if(start||end) newDates[key]={start:start||addAdGroupTarget.task.start,end:end||addAdGroupTarget.task.end};
          });
          const newTotal=Object.entries(newBudgets).filter(([k])=>!k.endsWith("__parent__")).reduce((a,[,b])=>a+b,0);
          updateCampaign(addAdGroupTarget.task.id,{channelBudgets:newBudgets,channelDates:newDates,budget:newTotal});
          adjustBank&&adjustBank(addAdGroupTarget.task.customerId,-totalAdded);
          setAddAdGroupTarget(null);
        }}
      />}
        ...
        return (
          <div key={customer.id}
            ...
            {!collapsedCustomers[customer.id]&&custTasks.map((task,taskIdx)=>(
              <TaskBlock key={task.id} task={task} taskIdx={taskIdx} custTasks={custTasks} accent={accent} updateCampaign={updateCampaign} deleteCampaign={deleteCampaign} navigate={navigate} adjustBank={adjustBank} teamMembers={teamMembers} onAddCampaign={(ch,existingTask)=>{
                if(existingTask) {
                  setAddLineTarget({task:existingTask,channel:ch});
                } else {
                  onAddCampaign(customer,{task,channel:ch});
                }
              }} onAddAdGroup={(target)=>setAddAdGroupTarget(target)} session={session} logActivity={logActivity}/>
            ))}
...
function TaskBlock({task, taskIdx, custTasks, accent, updateCampaign, deleteCampaign, navigate, adjustBank, onAddCampaign, onAddAdGroup, session, logActivity, teamMembers=[]}) {
  ...
  return (
    ...
    <div style={{borderBottom:"1px solid "+C.borderSoft}}>
      ...
      {(task.sharedWith?.length>0||(task.ownerId&&task.ownerId!==session?.user?.id))&&(
        <div style={{display:"flex",alignItems:"center",gap:8,flexWrap:"wrap",padding:"7px 14px",background:C.sandBg,borderBottom:"1px solid "+C.sandBd}}>
          {task.ownerId&&task.ownerId!==session?.user?.id&&(()=>{
            const owner=teamMembers.find(m=>m.id===task.ownerId);
            return <span style={{fontFamily:"Roboto,sans-serif",fontSize:11,color:C.sandDeep}}>Eies av: <strong>{owner?.display_name||owner?.email||"ukjent"}</strong></span>;
          })()}
          {task.sharedWith?.length>0&&(
            <>
              <span style={{fontFamily:"Roboto,sans-serif",fontSize:11,color:C.sandDeep}}>Delt med:</span>
              {task.sharedWith.map(uid=>{
                const member=teamMembers.find(m=>m.id===uid);
                const memberName=member?.display_name||member?.email||"ukjent";
                return (
                  <span key={uid} style={{display:"inline-flex",alignItems:"center",gap:4,background:C.card,border:"1px solid "+C.sandBd,borderRadius:99,padding:"2px 4px 2px 9px",fontFamily:"Roboto,sans-serif",fontSize:11,color:C.ink}}>
                    {memberName}
                    <button onClick={()=>{
                      if(confirm("Slutte å dele med "+memberName+"?")){
                        updateCampaign(task.id,{sharedWith:task.sharedWith.filter(id=>id!==uid)});
                      }
                    }} style={{background:"none",border:"none",cursor:"pointer",color:C.ink3,padding:3,display:"flex",borderRadius:"50%"}}><X size={10}/></button>
                  </span>
                );
              })}
            </>
...
function ShareCampaignsModal({custTasks, custName, onClose, onShare, onAssign}) {
  ...
  return (
    <div className="modal-overlay" onClick={e=>e.target===e.currentTarget&&onClose()}>
      <div className="modal">
        ...
        <div style={{display:"flex",gap:6,marginBottom:18,background:C.cardAlt,padding:4,borderRadius:9,border:"1px solid "+C.borderSoft}}>
          <button onClick={()=>setMode("share")} style={{flex:1,padding:"7px",borderRadius:7,border:"none",cursor:"pointer",fontFamily:"Roboto,sans-serif",fontSize:12.5,fontWeight:600,background:mode==="share"?C.sand:"transparent",color:mode==="share"?"#fff":C.ink3}}>Del (behold begge)</button>
          <button onClick={()=>setMode("assign")} style={{flex:1,padding:"7px",borderRadius:7,border:"none",cursor:"pointer",fontFamily:"Roboto,sans-serif",fontSize:12.5,fontWeight:600,background:mode==="assign"?C.sand:"transparent",color:mode==="assign"?"#fff":C.ink3}}>Tildel (overfør eierskap)</button>
        </div>
        ...
        {scope&&(
          <div>
            ...
            <div style={{fontFamily:"Roboto,sans-serif",fontSize:12,color:C.ink3,marginBottom:10}}>
              {mode==="share"?"Del":"Tildel"} {matchingTasks.length} kampanje{matchingTasks.length!==1?"r":""} ({scopeLabel}) {mode==="share"?"med":"til"}:
            </div>
            ...
            {matchingTasks.length>0&&(
              <div style={{display:"flex",flexWrap:"wrap",gap:6}}>
                {staffList.map(s=>(
                  <button key={s.id} className="action-btn" onClick={()=>mode==="share"?onShare(matchingTasks,s):onAssign(matchingTasks,s)} style={{fontFamily:"Roboto,sans-serif",fontSize:12}}>
                    {s.name}
                  </button>
                ))}
              </div>
...
function AddCampaignModal({customer, customers=[], presetChannel, onClose, onSave, tasks=[]}) {
  ...
  const toggleChannel=(ch)=>{
    setChannelLines(prev=>{
      ...
      return {...prev,[ch]:[{id:uid(),name:"",budget:"",restspend:"",useAdGroups:false,adGroups:[{id:uid(),name:"",budget:"",start:"",end:""}]}]};
    });
    setOpenCohort(null);
  };

  const addLine=(ch)=>setChannelLines(prev=>({...prev,[ch]:[...prev[ch],{id:uid(),name:"",budget:"",restspend:"",useAdGroups:false,adGroups:[{id:uid(),name:"",budget:"",start:"",end:""}]}]}));
